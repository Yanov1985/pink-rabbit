...
// Метаданные для SEO
useSeoMeta({
  title: "Каталог секс игрушек - Pink Rabbit | Интимные товары для взрослых",
  description:
    "Большой выбор качественных секс игрушек: для женщин, мужчин и пар. ✅ Быстрая доставка ✅ Дискретная упаковка ✅ Оригинальные товары",
  ogTitle: "Каталог секс игрушек - Pink Rabbit",
  ogDescription:
    "Широкий ассортимент интимных игрушек для взрослых. Качественные товары с быстрой доставкой по России.",
  ogImage: "/images/catalog-seks-igrushki.jpg",
  ogUrl: "https://pinkrabbit.ru/catalog",
  keywords:
    "секс игрушки, интимные товары, вибраторы, мастурбаторы, для взрослых, секс шоп",
});
...
// ДОБАВЛЯЕМ: Инициализируем composable для работы с каталогом
const { getSubcategories } = useCatalog();

// ДОБАВЛЯЕМ: Получаем главные категории для отображения
const mainCategories = computed(() => {
  console.log("🏠 Получение главных категорий для главной страницы каталога");
  const categories = getSubcategories([]); // Пустой путь для получения главных категорий
  console.log("📋 Главные категории получены:", categories.length);
  return categories;
});
